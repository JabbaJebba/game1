...
class WorldGenerator {
    constructor(width, height, rockType, seed = Math.random()) {
        this.width = width;
        this.height = height;
        this.rockType = rockType || { name: 'Stone' };
        ...
        this.TILE_GOLD = 6;
        this.TILE_RUBY = 7;
        this.TILE_SAPPHIRE = 8;
        this.TILE_EMERALD = 9;
        this.TILE_DIAMOND = 10;
        this.TILE_AMETHYST = 11;
        this.TILE_TITANIUM = 12;
        
        this.TILE_PLATINUM = 14;
    ...
    generate() {
        ...
        // Pass 1: Base terrain (grass → rock → bedrock). NO DIRT.
        for (let x = 0; x < this.width; x++) {
    ...
    generateVeins(surfaceY) {
        ...
        this.createVeins(this.TILE_PLATINUM, 5, surfaceY + 260, surfaceY + 400, 3, 6, 1, 2);
    ...
    paintVeinPoint(cx, cy, radius, type) {
        for (let dx = -radius; dx <= radius; dx++) {
            for (let dy = -radius; dy <= radius; dy++) {
                ...
                // Only replace rock (not grass, bedrock, or already-ore)
                if (this.tiles[x][y] !== this.TILE_ROCK) continue;
